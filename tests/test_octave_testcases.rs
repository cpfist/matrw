...
static MATFILE: Lazy<Mutex<MatFile>> = Lazy::new(|| {
    ...
    let filepath = TestFile {
        path: "tests/dynamic_octave_testcases.mat",
    };

    let out = OctaveCommand::run("tests/dynamic_testcases.m");
    println!("error output: {}", out);

    Mutex::new(load_matfile(filepath.path).unwrap_or_default())
});

macro_rules! load_octave_numeric_type {
    ($ty: ty, $var: expr, $ref: expr) => {
        paste! {
            #[test]
            fn [<load_octave_ $var>]() {
                skip_if_missing!("octave");
...
load_octave_numeric_type!(u8, "numeric_var_u8", vec![1, 3, 2, 4]);
load_octave_numeric_type!(i8, "numeric_var_i8", vec![-1, 3, 2, 4]);
load_octave_numeric_type!(u16, "numeric_var_u16", vec![1, 3, 2, 4]);
load_octave_numeric_type!(i16, "numeric_var_i16", vec![-1, 3, 2, 4]);
load_octave_numeric_type!(u32, "numeric_var_u32", vec![1, 3, 2, 4]);
load_octave_numeric_type!(i32, "numeric_var_i32", vec![-1, 3, 2, 4]);
load_octave_numeric_type!(u64, "numeric_var_u64", vec![1, 3, 2, 4]);
load_octave_numeric_type!(i64, "numeric_var_i64", vec![-1, 3, 2, 4]);
load_octave_numeric_type!(f32, "numeric_var_f32", vec![-1., 3., 2., 4.]);
load_octave_numeric_type!(f64, "numeric_var_f64", vec![-1., 3., 2., 4.]);
load_octave_numeric_type!(char, "numeric_var_char", vec!['1', '3', '2', '4']);

#[test]
fn load_octave_empty_struct() {
    skip_if_missing!("octave");
...
#[test]
fn load_octave_struct_array_with_empty() {
    skip_if_missing!("octave");

    let matfile = MATFILE.lock().unwrap();

    let d = &matfile["struct_arr"];
    println!("{:#?}", d);
}

#[test]
fn load_octave_struct_array_s() {
    skip_if_missing!("octave");
